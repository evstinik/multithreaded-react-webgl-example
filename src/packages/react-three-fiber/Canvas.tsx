...
export function Canvas({ eventSource, worker, fallback, style, className, id, ...props }: CanvasProps) {
  ...
  useEffect(() => {
    ...
    if ((canvas as any)?.transferedControl) {
      return
    }
    ...
    try {
      ...
      Object.assign(canvas, { transferedControl: true })
    } catch (e) {
      // Browser doesn't support offscreen canvas at all
      console.log("Browser doesn't support offscreen canvas at all", e)
      setFallback(true)
      return
    }
    ...
    worker.onmessage = (e) => {
      if (e.data.type === 'error') {
        ...
        console.log('Worker failed to initialize', e.data)
        setFallback(true)
      }
    }
